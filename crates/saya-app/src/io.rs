...
pub async fn watcher_io(
    ...
    if ocr_enabled {
        ...
        tokio::task::spawn_blocking(move || {
            println!(">>> [OCR] Starting hotkey listener...");

            let hotkey_manager = match saya_ocr::HotkeyManager::new_f9() {
                Ok(m) => m,
                Err(e) => {
                    println!(">>> [OCR] Failed to create hotkey: {}", e);
                    return;
                }
            };

            println!(">>> [OCR] F9 hotkey registered, polling...");
            ...
            loop {
                ...
                if hotkey_manager.poll() {
                    ...
                    tokio::spawn(async move {
                        println!(">>> [OCR] Starting async OCR flow...");
                        ...
                        let region = if let Some(ref title) = target_window {
                            ...
                            println!(">>> [OCR] Target window: {}", title);
                            ...
                            println!(">>> [OCR] No capture region configured!");
                        ...
                        println!(">>> [OCR] Capturing region: {}x{} at ({},{})",
                        ...
                        match result {
                            Ok(Ok((bytes, text))) => {
                                ...
                                if !text.trim().is_empty() {
                                    println!(">>> [OCR] Sending TextInput event...");
            ...
            println!(">>> [OCR] Hotkey listener stopped");
        });
    }
